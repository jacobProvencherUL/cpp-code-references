...
std::string EmployeHoraire::reqEmployeFormate() const
{
	...
	os << Employe::reqEmployeFormate() << std::endl; // appel de la méthode reqEmployeFormate de la classe parent Employe
	os << "Date de naissance : " << Employe::reqDateNaissance() << std::endl;
	os << "Code de departement : " << Employe::reqCodeDepartement() << std::endl;
	os << "Son taux horaire : " << m_tauxHoraire << std::endl;
	os << "Nombre d’heures effectuees : " << m_heures << std::endl;
	os << "Les gains : " << gains() << std::endl;
	return os.str();
}
